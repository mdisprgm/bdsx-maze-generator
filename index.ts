...
export class Structure {
    ...
    fill(
        ...
        y2: number,
        z2: number,
        blockId: string | null | ((x: number, y: number, z: number) => string | null),
    ): void {
    ...
    generate(x: number, y: number, z: number): { message: string; failed: number } {
        ...
        events.commandOutput.on(canceler);
            ...
            for (let yi = y; yi < ye; yi++) {
                for (let xi = x; xi < xe; xi++) {
                    ...
                    if (setblock(xi, yi, zi, block) !== 1) {
                        if (lastMessage !== "The block couldn't be placed") {
                            // same block
                            err.add(lastMessage);
                            failed++;
                        }
        ...
        events.commandOutput.remove(canceler);
        return {
...
export function generateMaze(sizeX: number, height: number, sizeZ: number, block: Block, pathWidth: number): Structure {
    ...
    class Wall {
        constructor(public readonly x: number, public readonly y: number, public readonly gp: GenPoint) {}
